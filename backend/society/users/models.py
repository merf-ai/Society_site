    ...
    for invalid_symbol in invalid_symbols:
        if invalid_symbol in value:
            raise ValidationError(
                "Логин содержит один из недопустимых символов: @, !, ., *, #"
            )
...
class Posts(models.Model):
    poster = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="user_poster"
    )
...
class Message(models.Model):
    sender = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="user_ms_sender"
    )
    reciever = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="user_ms_reciever"
    )
